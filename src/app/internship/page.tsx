...
export const metadata = {
  ...
  description:
    "Graduate and postgraduate psychology internships with mentorship and clinical exposure.",
};
...
const STEPS = [
  {
    icon: <ClipboardList className="w-6 h-6 text-teal-sage" />,
    title: "Submit Application",
  },
  {
    icon: <FileCheck className="w-6 h-6 text-teal-sage" />,
    title: "Application Review",
  },
  { icon: <Users className="w-6 h-6 text-teal-sage" />, title: "Interview" },
  {
    icon: <GraduationCap className="w-6 h-6 text-teal-sage" />,
      ...
      <section className="pt-36 pb-20">
        <Container className="text-center">
          ...
          <p className="text-charcoal/80 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed text-pretty">
            Learning opportunities, mentorship, and professional development for
            psychology students entering clinical or administrative practice.
          </p>
          <a
            href="#apply"
        ...
        <Container className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {PROGRAMS.map((p) => (
            ...
            >
              ...
              <h2 className="font-cormorant text-2xl font-semibold text-forest-slate mb-4">
                {p.title}
              </h2>
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs mb-6">
                <div>
                  <dt className="uppercase tracking-wide text-charcoal/80">
                    Duration
                  </dt>
              ...
              <h3 className="text-xs font-bold uppercase tracking-wide text-charcoal/80 mb-2">
                You&apos;ll Learn
              </h3>
        ...
        <Container>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-12 max-w-4xl mx-auto">
            {BENEFITS.map((b) => (
              <div
                key={b}
                className="flex items-center gap-2 text-sm text-forest-slate"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
            {STEPS.map((s, i) => (
              ...
              >
                ...
                <p className="font-cormorant text-lg font-semibold text-forest-slate">
                  {s.title}
                </p>
              </Surface>
            ))}
