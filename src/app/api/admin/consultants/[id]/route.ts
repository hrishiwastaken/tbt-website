...
import { handleApi, notFound, parseBody, requireAdmin, requireStaff } from "@/server/http";
import { consultantBalance } from "@/server/services/ledgerService";
import { getDefaultCommissionBps, setTherapistCommission } from "@/server/services/commissionService";
...
export const GET = handleApi(async (request: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  const [balance, defaultBps, payouts, recentBookings, earningsEntries, commissionHistory] =
    await Promise.all([
      consultantBalance(id),
      getDefaultCommissionBps(),
      prisma.payout.findMany({ where: { therapistId: id }, orderBy: { createdAt: "desc" }, take: 25 }),
      ...
      prisma.ledgerEntry.findMany({
        where: { therapistId: id, entryType: { in: ["COMMISSION_ACCRUED", "COMMISSION_REVERSED", "PAYOUT_PAID"] } },
...
export const PATCH = handleApi(async (request: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (body.feeRupees !== undefined) data.feeMinor = rupeesToMinor(body.feeRupees);
    ...
    if (body.availability) {
      await tx.therapistAvailability.deleteMany({ where: { therapistId: id } });
      if (body.availability.length > 0) {
        await tx.therapistAvailability.createMany({
          data: body.availability.map((slot) => ({ therapistId: id, ...slot })),
        });
      }
    }
