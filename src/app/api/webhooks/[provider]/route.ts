...
export const POST = handleApi(async (request: Request, ctx: { params: Promise<{ provider: string }> }) => {
  ...
  try {
    ...
    if (err instanceof Error && err.message.includes("Unknown payment provider")) {
