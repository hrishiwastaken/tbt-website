...
import { badRequest, clientIp, handleApi, notFound, parseBody, requireTherapist } from "@/server/http";
...
const THERAPIST_ALLOWED_TRANSITIONS = new Set(["COMPLETED", "NO_SHOW", "CANCELLED", "REFUND_PENDING"]);

async function loadOwnBooking(id: string, therapistId: string) {
  const booking = await prisma.booking.findFirst({ where: { id, therapistId } });
...
const patchSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("transition"), toStatus: z.string(), reason: z.string().max(500).optional() }),
...
export const PATCH = handleApi(async (request: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  if (body.action === "transition") {
    if (!isBookingStatus(body.toStatus) || !THERAPIST_ALLOWED_TRANSITIONS.has(body.toStatus)) {
      throw badRequest("Consultants may only mark sessions completed, no-show, cancelled, or request a refund");
    }
