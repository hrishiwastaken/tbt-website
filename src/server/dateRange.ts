...
export function resolveRange(
  ...
  switch (key) {
    ...
    case "90d":
      return {
        key,
        from: snapToBucketStart(addDays(now, -89), "week"),
        to,
        granularity: "week",
      };
    case "month":
    ...
    case "custom": {
      ...
      const spanDays = Math.ceil(
        (toDate.getTime() - from.getTime()) / 86400000,
      );
      const granularity: Granularity =
        spanDays <= 45 ? "day" : spanDays <= 200 ? "week" : "month";
      return {
        key,
        from: snapToBucketStart(from, granularity),
        to: toDate,
        granularity,
      };
    }
...
export function bucketKeys(
  ...
  while (cursor <= to) {
    ...
    cursor.setDate(
      cursor.getDate() +
        (granularity === "month" ? 28 : granularity === "week" ? 7 : 1),
    );
  }
